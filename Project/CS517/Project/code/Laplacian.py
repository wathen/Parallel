...
for xx in xrange(1,m):
    ...
    p = - inner(avg(grad(v)), outer(u('+'),N('+'))+outer(u('-'),N('-')))*dS \
        - inner(outer(v('+'),N('+'))+outer(v('-'),N('-')), avg(grad(u)))*dS \
        + alpha/h_avg*inner(outer(v('+'),N('+'))+outer(v('-'),N('-')),outer(u('+'),N('+'))+outer(u('-'),N('-')))*dS \
        - inner(outer(v,N), grad(u))*ds \
        - inner(grad(v), outer(u,N))*ds \
    ...
    PP = assemble(p)
    PP, bP = assemble_system(p, L1, bc)
    Ps = PP.sparray()
    Ps = Ps + 1e-2*sps.eye( Ps.shape[1])
    #plt.spy(Ps)
    ...
    if (Solving == 'Iterative'):
        ...
        ml = ruge_stuben_solver(As)               # construct the multigrid hierarchy
